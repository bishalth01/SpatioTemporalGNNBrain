...
from ABCD import preprocessabcd as Reader
...
HCP_DEMOGRAPHICS_PATH = '/data/users2/bthapaliya/spatioWithWANDB/labels_HCP_Gender.csv'

ABCD_DEMOGRAPHICS_PATH = '/data/users2/bthapaliya/BrainGNNWithdFNCGender/data/SubjectInfo.txt'
...
def get_desikan_ts_path(person: int, direction: str):
    return  np.load('/data/users2/umahmood1/ICABrainGNN/BrainGNN/DataandLabels/HCP_AllData.npz')[person]
    #return np.load(f'/data/users2/bthapaliya/spatiotemporal_GNN/data/HCP_1200/hcp_tc_npy_22/{person}_cortex.npy')
    #return nib.load(f'/data/qneuromark/Data/HCP/Data_BIDS/Preprocess_Data/{person}/rfMRI_REST{direction}/func')


def get_desikan_tracks_path_abcd(person: int):
    return f'../hcp_data/desikan_tracks/{person}_conn_aparc+aseg_RS_sl.txt'


def get_desikan_ts_path_abcd(person: int, direction: str):
    ...
    path = '/data/users2/harshitha/Input_Data/Output/Output'+person+'.mat'
    matrix = sio.loadmat(path)['output'][:50]
    matrix = matrix.T
    matrix = matrix.reshape(53, 53*50)
    return matrix
    #return nib.load(f'/data/qneuromark/Data/HCP/Data_BIDS/Preprocess_Data/{person}/rfMRI_REST{direction}/func')
...
class ABCDDataset(BrainDataset):
    def __init__(self, root, target_var: str, num_nodes: int, threshold: int, connectivity_type: ConnType,
        ...
        super(ABCDDataset, self).__init__(root, target_var=target_var, num_nodes=num_nodes, threshold=threshold,
    ...
    def processed_file_names(self):
        return ['data_abcd_brain.dataset']
    ...
    def __create_data_object(self, person: int, ts: np.ndarray, ind: int, edge_attr: torch.Tensor,
        ...
        if self.target_var == 'gender':
            if (self.info_df.loc[self.info_df["mat_numbers"]==int(person)].values[0][2]=='F'):
                y= torch.tensor(0, dtype=torch.float)
            else:
                y= torch.tensor(1, dtype=torch.float)
            #y = torch.tensor([self.info_df.loc[self.info_df["mat_numbers"]==int(person)]], dtype=torch.float)
            #y = torch.tensor([self.info_df.loc[self.info_df.loc['mat_']], dtype=torch.float)
        data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, y=y)
        data.abcd_id = torch.tensor([int(person)])
        data.index = torch.tensor([ind])

        return data

    def _get_struct_arr(self, person):
        arr_struct = np.genfromtxt(get_desikan_tracks_path_abcd(person))
    ...
    def process(self):
        ...
        filtered_people = Reader.get_ids()
        labels = Reader.get_subject_score(filtered_people, score='sex')
        matnumbers = Reader.get_subject_score(filtered_people, score='SubjectId')
        num_classes = 2
        ...
        for person in filtered_people:
            ...
            for ind, direction in enumerate(['1_LR']):
                ts = get_desikan_ts_path_abcd(person, direction)
                # Because of normalisation part
                ts = ts.T
                ts = ts[:, self._idx_to_filter]
                assert ts.shape[0] == 1185
                assert ts.shape[1] == 100#68

                # Crop timeseries
                if self.time_length != 100*1185:
...
class HCPDataset(BrainDataset):
    def __init__(self, root, target_var: str, num_nodes: int, threshold: int, connectivity_type: ConnType,
        ...
        # arr_struct will only have values in the upper triangle
        #self._idx_to_filter = np.concatenate((np.arange(0, 34), np.arange(49, 83)))
        self._idx_to_filter = np.arange(0, 100)
    ...
    @property
    def processed_file_names(self):
        return ['data_hcp_brain_custom.dataset']
    ...
    def process(self):
        # Read data into huge `Data` list.
        data_list: List[Data] = []
        assert self.time_length == 1185 or self.time_length == 490
        ...
        for person in filtered_people:
            ...
            for ind, direction in enumerate(['1_LR']):
                ts = get_desikan_ts_path(person, direction)
                # Because of normalisation part
                ts = ts.T
                ts = ts[:, self._idx_to_filter]
                assert ts.shape[0] == 1185
...
class FlattenCorrsDataset(InMemoryDataset):
    def __init__(self, root, num_nodes: int, connectivity_type: ConnType, analysis_type: AnalysisType, time_length: int,
        ...
        if analysis_type not in [AnalysisType.FLATTEN_CORRS]:
            ...
            exit(-2)
        if dataset_type not in [DatasetType.UKB, DatasetType.HCP, DatasetType.ABCD]:
    ...
    def __get_abcd_data_object(self, person: int, direction: str, ind: int) -> Data:
        info_df = pd.read_csv(ABCD_DEMOGRAPHICS_PATH).set_index('mat_numbers')

        idx_to_filter = np.arange(0,53)
        ts = np.genfromtxt(get_desikan_ts_path_abcd(person, direction))

        ts = ts.T
        ts = ts[:, idx_to_filter]
        assert ts.shape[0] == 1200
        #assert ts.shape[1] == 68
        assert ts.shape[1] == 22

        flatten_array = self.__generate_flatten_from_ts(ts)
        x = torch.tensor(flatten_array, dtype=torch.float)

        data = Data(x=x)
        data.abcd_id = torch.tensor([person])
        data.index = torch.tensor([ind])
    ...
    def process(self):
        ...
        if self.dataset_type == DatasetType.UKB:
            filtered_people = np.load(UKB_IDS_PATH)
        elif self.dataset_type == DatasetType.ABCD:
            filtered_people = Reader.get_ids()
        else:
            filtered_people = sorted(list(set(DESIKAN_COMPLETE_TS).intersection(set(DESIKAN_TRACKS))))
        ...
        for person in filtered_people:
            ...
            if self.dataset_type == DatasetType.UKB:
                ...
                for ind, direction in enumerate(['1_LR']):
                    data = self.__get_abcd_data_object(person, ind=ind, direction=direction)
                    data_list.append(data)
            else:  # HCP
