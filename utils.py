...
def create_name_for_flattencorrs_dataset(config_params: Dict[str, Any]) -> str:
    prefix_location = './pytorch_data/unbalanced_'

    name_combination = '_'.join([config_params['dataset_type'].value,
                                 config_params['analysis_type'].value,
                                 config_params['param_conn_type'].value,
                                 str(config_params['num_nodes']),
                                 str(config_params['time_length'])
                                 ])
...
def create_name_for_xgbmodel(config_params: Dict[str, Any], outer_split_num: int, model: XGBModel, inner_split_num: int,
                             prefix_location='logs/', suffix='.pkl') -> str:
    if config_params['analysis_type'] == AnalysisType.FLATTEN_CORRS:
        model_str_representation = config_params['analysis_type'].value
    ...
    return prefix_location + '_'.join([config_params['target_var'],
                                       config_params['dataset_type'].value,
                                       str(outer_split_num),
                                       str(inner_split_num),
...
def create_name_for_model(config_params: Dict[str, Any], model, outer_split_num: int, inner_split_num: int,
                          prefix_location='logs/', suffix='.pt') -> str:
    if config_params['analysis_type'] in [AnalysisType.ST_MULTIMODAL, AnalysisType.ST_UNIMODAL, AnalysisType.ST_UNIMODAL_AVG, AnalysisType.ST_MULTIMODAL_AVG]:
        model_str_representation = model.to_string_name()

    lr = round(config_params['param_lr'], 7)
    weight_decay = round(config_params['param_weight_decay'], 7)
    ...
    return prefix_location + '_'.join([config_params['target_var'],
                                       ...
                                       str(lr),
                                       str(weight_decay),
                                       str(config_params['param_threshold']),
                                       config_params['param_normalisation'].value[:3],
                                       str(config_params['num_nodes']),
                                       config_params['param_conn_type'].value
                                       ]) + suffix
