...
class GNN(torch.nn.Module):
    def __init__(self,
        ...
        self.dp_norm = config_params['dp_norm']

        if config_params['dp_norm'] == 'batchnorm':
            self.bn1 = torch.nn.BatchNorm1d(hidden_channels)
            self.bn2 = torch.nn.BatchNorm1d(hidden_channels)
            self.bn3 = torch.nn.BatchNorm1d(out_channels)
        elif config_params['dp_norm'] == 'graphnorm':
            self.bn1 = GraphNorm(hidden_channels)
            self.bn2 = GraphNorm(hidden_channels)
            self.bn3 = GraphNorm(out_channels)
        elif config_params['dp_norm'] == 'graphsizenorm':
...
class DiffPoolLayer(torch.nn.Module):
    def __init__(self, max_num_nodes, num_init_feats, aggr, config_params):
        ...
        num_nodes = max(1, ceil(config_params['dp_perc_retaining'] * self.max_nodes))
        self.gnn1_pool = GNN(self.init_feats, self.INTERN_EMBED_SIZE, num_nodes, aggr=aggr, config_params=config_params)
        self.gnn1_embed = GNN(self.init_feats, self.INTERN_EMBED_SIZE, self.INTERN_EMBED_SIZE, lin=False, aggr=aggr, config_params=config_params)

        num_nodes = max(1, ceil(config_params['dp_perc_retaining'] * num_nodes))
        self.final_num_nodes = num_nodes
        self.gnn2_pool = GNN(3 * self.INTERN_EMBED_SIZE, self.INTERN_EMBED_SIZE, num_nodes, aggr=aggr, config_params=config_params)
        self.gnn2_embed = GNN(3 * self.INTERN_EMBED_SIZE, self.INTERN_EMBED_SIZE, self.INTERN_EMBED_SIZE, lin=False, aggr=aggr, config_params=config_params)

        self.gnn3_embed = GNN(3 * self.INTERN_EMBED_SIZE, self.INTERN_EMBED_SIZE, self.INTERN_EMBED_SIZE, lin=False, aggr=aggr, config_params=config_params)
        if self.aggr == 'improved':
...
class PNANodeModel(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features, activation, config_params):
        super(PNANodeModel, self).__init__()

        if config_params['nodemodel_aggr'] == 'all':
            aggregators = ['mean', 'min', 'max', 'std', 'sum']
        else:
            aggregators = [config_params['nodemodel_aggr']]

        if config_params['nodemodel_scalers'] == 'all':
        ...
        for _ in range(config_params['nodemodel_layers']):
            conv = PNAConv(in_channels=num_node_features, out_channels=num_node_features,
                           aggregators=aggregators, scalers=scalers, deg=config_params['dataset_indegree'],
...
class SpatioTemporalModel(nn.Module):
    def __init__(self, config_params: Dict[str, Any],
        ...
        num_time_length = config_params['time_length']
        dropout_perc = config_params['param_dropout']
        pooling = config_params['param_pooling']
        channels_conv = config_params['param_channels_conv']
        activation = config_params['param_activation']
        conv_strategy = config_params['param_conv_strategy']
        sweep_type = config_params['sweep_type']
        gat_heads = config_params['param_gat_heads']
        edge_weights = config_params['edge_weights']
        final_sigmoid = config_params['model_with_sigmoid']
        num_nodes = config_params['num_nodes']
        num_gnn_layers = config_params['param_num_gnn_layers']
        encoding_strategy = config_params['param_encoding_strategy']
        multimodal_size = config_params['multimodal_size']
        temporal_embed_size = config_params['temporal_embed_size']
        ...
        if self.sweep_type == SweepType.GCN:
            ...
            self.meta_layer = MetaLayer(edge_model=EdgeModel(num_node_features=self.NODE_EMBED_SIZE,
                                        ...
                                        node_model=PNANodeModel(num_node_features=self.NODE_EMBED_SIZE, num_edge_features=1,
                                                                activation=self.activation, config_params=config_params))
        ...
        if self.conv_strategy == ConvStrategy.TCN_ENTIRE:
            ...
            if config_params['tcn_hidden_units'] == 8:
                self.size_before_lin_temporal = self.channels_conv * (2 ** (config_params['tcn_depth'] - 1)) * self.num_time_length
            else:
                self.size_before_lin_temporal = config_params['tcn_hidden_units'] * self.num_time_length

            self.lin_temporal = self._get_lin_temporal(config_params)

            tcn_layers = []
            for i in range(config_params['tcn_depth']):
                if config_params['tcn_hidden_units'] == 8:
                    tcn_layers.append(self.channels_conv * (2 ** i) )
                else:
                    tcn_layers.append(config_params['tcn_hidden_units'])

            self.temporal_conv = TemporalConvNet(1,
                                                 tcn_layers,
                                                 kernel_size=config_params['tcn_kernel'],
                                                 dropout=self.dropout,
                                                 norm_strategy=config_params['tcn_norm_strategy'])
        elif self.conv_strategy == ConvStrategy.LSTM:
            self.temporal_conv = nn.LSTM(input_size=1,
                                         hidden_size=config_params['tcn_hidden_units'],
                                         num_layers=config_params['tcn_depth'],
                                         dropout=dropout_perc,
                                         batch_first=True)
            ...
            def init_lstm_hidden(x):
                h0 = torch.zeros(config_params['tcn_depth'], x.size(0), config_params['tcn_hidden_units'])
                c0 = torch.zeros(config_params['tcn_depth'], x.size(0), config_params['tcn_hidden_units'])
        ...
        if self.pooling == PoolingStrategy.DIFFPOOL:
            ...
            self.diff_pool = DiffPoolLayer(num_nodes, self.NODE_EMBED_SIZE, aggr='mean', config_params=config_params)
            ...
            if self.pooling == PoolingStrategy.DP_MAX:
                self.diff_pool = DiffPoolLayer(num_nodes, self.NODE_EMBED_SIZE, aggr='max', config_params=config_params)
            elif self.pooling == PoolingStrategy.DP_ADD:
                self.diff_pool = DiffPoolLayer(num_nodes, self.NODE_EMBED_SIZE, aggr='add', config_params=config_params)
            elif self.pooling == PoolingStrategy.DP_MEAN:
                self.diff_pool = DiffPoolLayer(num_nodes, self.NODE_EMBED_SIZE, aggr='mean', config_params=config_params)
            elif self.pooling == PoolingStrategy.DP_IMPROVED:
                self.diff_pool = DiffPoolLayer(num_nodes, self.NODE_EMBED_SIZE, aggr='improved', config_params=config_params)

        if config_params['final_mlp_layers'] == 1:
            self.final_linear = nn.Linear(self.NODE_EMBED_SIZE, 1)
        elif config_params['final_mlp_layers'] == 2:
    ...
    def _get_lin_temporal(self, config_params):
        if config_params['tcn_final_transform_layers'] == 1:
            lin_temporal = nn.Linear(self.size_before_lin_temporal,
                                          self.NODE_EMBED_SIZE - self.multimodal_size)
        elif config_params['tcn_final_transform_layers'] == 2:
        ...
        elif config_params['tcn_final_transform_layers'] == 3:
            lin_temporal = nn.Sequential(
                nn.Linear(self.size_before_lin_temporal, int(self.size_before_lin_temporal / 2)),
                self.activation, nn.Dropout(self.dropout),
